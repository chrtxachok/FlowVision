...
Единый контракт:
  - вход: bytes файла + MIME-тип
  - выход: `np.ndarray` в формате BGR (OpenCV)

Это важно, потому что:
  - большинство операций улучшения изображения в проекте сделаны на OpenCV;
  - OCR backend'ы получают уже нормализованное изображение (после `enhance_image`).
"""
import io
import logging
import numpy as np
...
def enhance_image(img: np.ndarray) -> np.ndarray:
    ...
    Примечание:
      - Мы возвращаем BGR даже после преобразований в grayscale, чтобы downstream код
        (и PaddleOCR) не требовал ветвления по числу каналов.
    """
